...
def iter_month_starts(year: int):
    now = datetime.datetime.utcnow()
    m = datetime.datetime(year, 1, 1)
    while m <= now:
        yield m
        # next month
        if m.month == 12: m = datetime.datetime(m.year + 1, 1, 1)
        else:             m = datetime.datetime(m.year, m.month + 1, 1)
        if m.year > year and m > now: break
...
# ---------- fetch closed tickets (single pass) ----------
...
# ---------- CSAT (best-effort; skip gracefully if not available) ----------
def try_fetch_csat_points_2025():
    """
    Return (year_avg, {month_start: avg}) if happiness is available.
    We attempt a generic endpoint; if 404/401, we skip.
    """
    try:
        ...
        r.raise_for_status()
        data = r.json()
        # Accept either a dict aggregate or a list of records with date+score/percent
        vals_year = []
        per_month_vals = defaultdict(list)

        if isinstance(data, dict) and "ratings" in data:
            # e.g. {"ratings":[{"date":"2025-01-15","percent":92.0}, ...]}
            data = data["ratings"]

        if isinstance(data, dict) and "average" in data:
            # Already aggregated for the whole period
            year_avg = float(data["average"])
            return year_avg, {}

        if isinstance(data, list):
            for row in data:
                # Try common fields: date or created_at; percent or score
...
# ---------- build Databox payload ----------
def slugify(s): return "".join(c.lower() if c.isalnum() else "_" for c in s).strip("_")

payload = [
    {"key":"enchant_resolved_2025_total",         "value": resolved_total},
    {"key":"enchant_resolution_minutes_avg_2025", "value": avg_resolution_year},
]
...
# monthly: counts + avg resolution + csat (each point carries a date)
for mon in sorted(m_counts.keys()):
    payload.append({
        "key":  "enchant_resolved_monthly",
        "value": int(m_counts[mon]),
        "date": iso_z(mon)  # first day of the month
    })
...
if csat_monthly:
    for mon in sorted(csat_monthly.keys()):
        payload.append({
            "key":  "enchant_csat_avg_monthly",
            "value": float(csat_monthly[mon]),
            "date": iso_z(mon)
        })
